...
CREATE INDEX idx_photo_listing_id ON photo(listing_id);
...
CREATE INDEX idx_listing_category_cat ON listing_category(category_id);
...
CREATE INDEX idx_rental_listing ON rental(listing_id);
CREATE INDEX idx_rental_lessor  ON rental(lessor_id);
CREATE INDEX idx_rental_lessee  ON rental(lessee_id);
CREATE INDEX idx_rental_status  ON rental(status);
CREATE INDEX idx_rental_period  ON rental(start_at, end_at);
...
CREATE TABLE report (
                        ...
                        target_type  VARCHAR(30),                         -- e.g. 'listing' | 'rental' | ...
                        target_id    UUID REFERENCES rental(id),          -- согласно модели — ссылка на rental
                        reason_body  TEXT,
                        created_at   TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX IF NOT EXISTS idx_rental_listing_start_end ON rental (listing_id, start_at, end_at);
CREATE INDEX IF NOT EXISTS idx_rental_start_end ON rental (start_at, end_at);